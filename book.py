...
btn_bk3 = types.InlineKeyboardButton("Истории успеха")
btn_bk4 = types.InlineKeyboardButton("Тайм менеджмент")
...
btn_back = types.InlineKeyboardButton('Вернуться на главную ↩️')
markup_pr.add(btn_pr1, btn_pr2, btn_back)
...
btn_back = types.InlineKeyboardButton('Вернуться на главную ↩️')
markup_ch.add(btn_ch1, btn_ch2, btn_back)
...
def handle_text(message):
    if message.text == 'Электронные книги 📚':
        ...
        document = open("mybot/book2.pdf", 'rb')
        bot.send_chat_action(message.from_user.id, 'upload_document')
        bot.send_document(message.from_user.id, document)
        document.close()
    elif message.text == 'Гарри Поттер':
        document = open("mybot/book6.pdf",'rb')
        bot.send_chat_action(message.from_user.id, 'upload_document')
        bot.send_document(message.from_user.id, document)
        document.close()
    #Истории успеха
    elif message.text == 'Истории успеха':
        bot.send_message(message.from_user.id, 'Выбери одну', reply_markup=markup_pr)
    elif message.text == 'Самый богатый человек в Вавилоне':
        document = open("mybot/book12.pdf", 'rb')
        bot.send_chat_action(message.from_user.id, 'upload_document')
        bot.send_document(message.from_user.id, document)
        document.close()
    elif message.text == 'Монарх, который продал свой Ferrari':
        document = open("mybot/book11.pdf", 'rb')
        bot.send_chat_action(message.from_user.id, 'upload_document')
        bot.send_document(message.from_user.id, document)
        document.close()
        # Тайм менеджмент
        ...
        document = open("mybot/book9.pdf", 'rb')
        bot.send_chat_action(message.from_user.id, 'upload_document')
        bot.send_document(message.from_user.id, document)
        document.close()
    elif message.text == 'Мастер времени. Трейси':
        document = open("mybot/book10.pdf",'rb')
        bot.send_chat_action(message.from_user.id, 'upload_document')
        bot.send_document(message.from_user.id, document)
        document.close()
    #Книги по психологии
        ...
        document = open("mybot/book8.pdf", 'rb')
        bot.send_chat_action(message.from_user.id, 'upload_document')
        bot.send_document(message.from_user.id, document)
        document.close()
    elif message.text == 'Бывшие. Наталья Краснова':
        document = open("mybot/book5.pdf", 'rb')
        bot.send_chat_action(message.from_user.id, 'upload_document')
        bot.send_document(message.from_user.id, document)
        document.close()
    elif message.text == 'Сила воли':
        document = open("mybot/book13.pdf", 'rb')
        bot.send_chat_action(message.from_user.id, 'upload_document')
        bot.send_document(message.from_user.id, document)
        document.close()
        ...
        document = open("mybot/book4.pdf", 'rb')
        bot.send_chat_action(message.from_user.id, 'upload_document')
        bot.send_document(message.from_user.id, document)
        document.close()
    elif message.text == 'Шұғаның белгісі. Бейімбет Майлин':
        document = open("mybot/book14.pdf", 'rb')
        bot.send_chat_action(message.from_user.id, 'upload_document')
        bot.send_document(message.from_user.id, document)
        document.close()
        ...
        audio = open('mybot/audio2.mp3', 'rb')
        bot.send_chat_action(message.from_user.id, 'upload_audio')
        bot.send_audio(message.from_user.id, audio)
        audio.close()

    elif message.text == "Магия утра":
        audio = open('mybot/audio1.mp3', 'rb')
        bot.send_chat_action(message.from_user.id, 'upload_audio')
        bot.send_audio(message.from_user.id, audio)
        audio.close()
